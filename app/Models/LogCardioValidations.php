...
class LogCardioValidations extends Model
{
    ...
    protected $appends = ['hr_max'];

    public function getHrMaxAttribute($value)
    {
        $userId = \Auth::id();
        $hrMaxObj = SettingTraining::where('user_id', $userId)->first();
        // $hrMax = (isset($hrMaxObj)) ? $hrMaxObj->hr_max : 0;
        if (isset($hrMaxObj)) {
            $hrMax = $hrMaxObj->hr_max;
        } else {
            /** if not found the customize */
            $authController = app(AuthController::class);
            $userBirthDate = \Auth::user()->date_of_birth;
            $authController->generateHRMaxFromDateOfBirth($userBirthDate, $userId);
            $hrMaxObj = SettingTraining::where('user_id', $userId)->first();
            $hrMax = $hrMaxObj->hr_max;
        }
        return $this->attributes['hr_max'] = $hrMax;
    }
